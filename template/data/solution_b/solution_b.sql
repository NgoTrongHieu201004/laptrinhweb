...
SELECT orders.user_id, users.user_name, orders.order_id, GROUP_CONCAT(products.product_name) AS products
FROM orders
JOIN users ON orders.user_id = users.user_id
JOIN order_details ON orders.order_id = order_details.order_id
...
-- 8. Liệt kê mỗi user chỉ chọn ra 1 đơn hàng có giá tiền lớn nhất

-- 9. Liệt kê mỗi user chỉ chọn ra 1 đơn hàng có giá tiền nhỏ nhất

-- 10. Liệt kê mỗi user chỉ chọn ra 1 đơn hàng có số sản phẩm nhiều nhất
